...
class UnifiedOrchestrator:
    ...
    def process_api_job(self, api_card_data: Dict[str, Any], dry_run: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'status': 'completed',
                'job_id': job.job_id,
                'outputs': result.google_storage_output_path
            }
    ...
    def _execute_job(self, job: BaseJob, workflow_manager) -> JobResult:
        ...
        try:
            ...
            # Wait for completion
            success, _err = self.comfyui_client.wait_for_prompt_completion(prompt_id, timeout=38000)
            ...
            if success:
                ...
                try:
                    ...
                    if storage.upload_file_to_gcs(str(artifact_path), gcs_path):
                        ...
                        logger.info(f"Google storage output path: {google_storage_output_path}")
                        artifact_path_nobg = job.get_artifact_full_path()+'_nobg'
                        logger.info(f"Artifact path (NO BG): " + artifact_path_nobg)
                        if storage.upload_file_to_gcs(str(artifact_path_nobg), gcs_path):
                            logger.info(f"Artifact path (NO BG): upload sucessful")

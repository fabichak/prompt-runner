...
class ComfyUIClient:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            self.slack_client.send_message(f"Connected to ComfyUI server at {self.server_address}")
            return True
        except Exception as e:
            logger.error(f"Failed to connect to ComfyUI: {e}")
            logger.error(f"WebSocket URL: {ws_url}")
            self.slack_client.send_message(f"Failed to connect to ComfyUI: {e}")
    ...
    def disconnect(self):
        ...
        if self.ws:
            ...
            self.slack_client.send_message(f"Disconnected from ComfyUI server")
    ...
    def queue_prompt(self, workflow: Dict[str, Any], prompt_id: Optional[str] = None) -> Optional[str]:
        ...
        try:
            ...
            with urllib.request.urlopen(req) as response:
                ...
                self.slack_client.send_message(f"Queued prompt {prompt_id}")
                return prompt_id
                
        except Exception as e:
            logger.error(f"Error queuing prompt: {e}")
            self.slack_client.send_message(f"Error queuing prompt: {e}")
            return None
    ...
    def wait_for_prompt_completion(self, prompt_id: str, timeout: int = 3600) -> Tuple[bool, Optional[str]]:
        ...
        if not self.ws:
            ...
            self.slack_client.send_message(f"WebSocket not connected")
            return False, "WebSocket not connected"
        
        logger.info(f"Waiting for prompt {prompt_id} to complete...")
        self.slack_client.send_message(f"Waiting for prompt {prompt_id} to complete...")
        start_time = time.time()
        
        while True:
            if time.time() - start_time > timeout:
                logger.error(f"Timeout waiting for prompt {prompt_id}")
                self.slack_client.send_message(f"Timeout waiting for prompt {prompt_id}")
                ...
                if isinstance(message, str):
                    ...
                    if data.get('type') == 'executing':
                        ...
                        if node_id is None and data['data'].get('prompt_id') == prompt_id:
                            ...
                            self.slack_client.send_message(f"Prompt {prompt_id} completed successfully")
                            return True, None
                        ...
                        if data['data'].get('prompt_id') == prompt_id:
                            ...
                            self.slack_client.send_message(f"Prompt {prompt_id} failed: {error_msg}")
                        ...
                        if data['data'].get('prompt_id') == prompt_id:
                            ...
                            self.slack_client.send_message(f"Progress {node}: {value}/{max_val}")
                            
            except websocket.WebSocketTimeoutException:
                continue
            except websocket.WebSocketConnectionClosedException:
                logger.error(f"WebSocket connection closed unexpectedly")
                self.slack_client.send_message(f"WebSocket connection closed unexpectedly")
                return False, "Connection closed"
            except Exception as e:
                logger.error(f"Error receiving message: {e}")
                self.slack_client.send_message(f"Error receiving message: {e}")
                return False, str(e)
    ...
    def get_history(self, prompt_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.slack_client.send_message(f"Error getting history for {prompt_id}: {e}")
            return None
    ...
    def interrupt_execution(self):
        ...
        try:
            ...
            self.slack_client.send_message(f"Interrupted execution")
        except Exception as e:
            logger.error(f"Error interrupting execution: {e}")
            self.slack_client.send_message(f"Error interrupting execution: {e}")
    ...
    def get_queue_status(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.slack_client.send_message(f"Error getting queue status: {e}")
            return None
    ...
    def clear_queue(self):
        ...
        try:
            ...
            self.slack_client.send_message(f"Cleared execution queue")
        except Exception as e:
            logger.error(f"Error clearing queue: {e}")
            self.slack_client.send_message(f"Error clearing queue: {e}")
    ...
    def execute_with_retry(self, workflow: Dict[str, Any], max_retries: int = MAX_RETRIES) -> Tuple[bool, Optional[str], Optional[str]]:
        ...
        for attempt in range(max_retries):
            ...
            if not prompt_id:
                ...
                self.slack_client.send_message(f"Failed to queue prompt, attempt {attempt + 1}/{max_retries}")
            ...
            self.slack_client.send_message(f"Execution failed (attempt {attempt + 1}/{max_retries}): {error}")
